...
violations="$(
  find "$charts_dir" -mindepth 1 -maxdepth 1 -type d | sort | while IFS= read -r chart_dir; do
    [ "$(basename "$chart_dir")" = "common" ] && continue   # the helper's home chart
    tmpl="$chart_dir/templates"
    [ -d "$tmpl" ] || continue
    # Only the chart's own templates — never vendored subcharts (charts/*/charts/).
    find "$tmpl" -name '*.yaml' -type f | sort | while IFS= read -r file; do
      awk -v file="$file" '
